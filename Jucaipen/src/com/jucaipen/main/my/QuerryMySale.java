 ...
 * 
 *         typeId 0 视频 1 专辑 2 日志观点 3 直播 4 问答 5 战法
 */
...
public class QuerryMySale extends HttpServlet {
	...
	private String initMySaleInfo(int uId, int type, int p) {
		if (type == 0) {
			...
			//2 日志观点
			List<IdeaSale> ideaSales = IdeaSaleServer.findTxtLiveSaleByUid(uId, p);
			...
			//3 直播
			List<LiveDetailSale> txtSales = LiveDetailSaleSer.findSaleByUid(uId,
					p);
			for(LiveDetailSale detailSale : txtSales){
				int detailId=detailSale.getLiveDetailId();
				int tId=detailSale.getTeacherId();
				TxtLiveDetails details = TxtLiveDetaileSer.findTextDetaileById(detailId);
			...
			for(AnswerSale answerSale : answerSales){
				...
				FamousTeacher teacher=FamousTeacherSer.findFamousTeacherById(answerSale.getTeacherId());
